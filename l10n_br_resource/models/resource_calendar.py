...
class ResourceCalendar(models.Model):
    ...
    def is_bank_holiday(self, reference_date):
        ...
        # leave_ids already aggregates this calendar and every ancestor, which is the
        # country/state/city chain. Searching resource.calendar.leaves globally instead
        # would match a holiday of any other municipality loaded in the database and
        # report a working day as a holiday.
        return len(
            self.leave_ids.filtered(
                lambda leave: leave.leave_type in ("F", "B")
                and leave.date_from <= reference_date <= leave.date_to
            )
        )
