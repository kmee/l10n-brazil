...
def post_init_hook(env):
    ...
    br_demo_companies = [env.ref("l10n_br_base.empresa_simples_nacional"), env.ref("l10n_br_base.empresa_lucro_presumido")]
    todo_companies = []
    for company in env["res.company"].with_context(active_test=False).search([]):
        ...
        elif company in br_demo_companies:
            env["account.chart.template"].try_loading("generic_coa", company)
            todo_companies.append(company)

    main_company = env.ref("base.main_company")
    if main_company not in todo_companies and env.ref("base.module_l10n_br_account").demo:
