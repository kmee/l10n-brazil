...
class DocumentWorkflow(models.AbstractModel):
    ...
    DEPRECATED: this is the compatibility layer of the state machine engine
    implemented in l10n_br_fiscal_edi/models/document.py (strangler fig
    pattern). The engine is the single source of truth for the allowed
    transitions and for the new `_before_document_*`/`_after_document_*`
    callbacks, while this mixin keeps the legacy interface (used by the
    l10n_br_* transmission modules, by the municipal NFS-e providers and by
    third party modules) working and plugged at the very same point of the
    flow. Modules are migrated to the new API one at a time; this mixin will
    be removed once no consumer uses the legacy API anymore.
    """
    ...
    def _avaliable_transition(self, old_state, new_state):
        """Verifica as transições disponiveis, para não permitir alterações
         de estado desconhecidas.

        Valida contra as tuplas legadas WORKFLOW_EDOC /
        WORKFLOW_DOCUMENTO_NAO_ELETRONICO, exatamente como na 16.0: este é
        um símbolo da API legada e módulos escritos contra ela dependem das
        transições legadas serem recusadas do mesmo jeito (ex.: reenviar uma
        NF-e rejeitada pelo caminho legado é um no-op filtrado, não uma
        transição). A máquina de estados declara um SUPERCONJUNTO dessas
        arestas; as arestas novas são alcançáveis apenas pela API nova
        (_trigger_fsm), que valida na própria máquina e não passa por aqui.
    ...
    def _before_change_state(self, old_state, new_state):
        """Hook para realizar alterações antes da alteração do estado do doc.

        Também dispara os callbacks `before` declarados pela máquina de
        estados para a transição em questão, de forma que a API nova e a
        legada sejam chamadas no mesmo ponto do fluxo. Um retorno falso do
        hook legado veta a transição (e, por consequência, os callbacks
        novos não são executados).
        ...
        if new_state == SITUACAO_EDOC_EM_DIGITACAO:
            ...
            result = self._exec_before_SITUACAO_EDOC_A_ENVIAR(old_state, new_state)
        elif new_state == SITUACAO_EDOC_ENVIADA:
            result = self._exec_before_SITUACAO_EDOC_ENVIADA(old_state, new_state)
        elif new_state == SITUACAO_EDOC_REJEITADA:
            result = self._exec_before_SITUACAO_EDOC_REJEITADA(old_state, new_state)
        elif new_state == SITUACAO_EDOC_AUTORIZADA:
            result = self._exec_before_SITUACAO_EDOC_AUTORIZADA(old_state, new_state)
        elif new_state == SITUACAO_EDOC_CANCELADA:
            result = self._exec_before_SITUACAO_EDOC_CANCELADA(old_state, new_state)
        elif new_state == SITUACAO_EDOC_DENEGADA:
            result = self._exec_before_SITUACAO_EDOC_DENEGADA(old_state, new_state)
        elif new_state == SITUACAO_EDOC_INUTILIZADA:
            result = self._exec_before_SITUACAO_EDOC_INUTILIZADA(old_state, new_state)
        if result:
            self._run_fsm_callbacks(old_state, new_state, "before")
        return result
    ...
    def _exec_after_SITUACAO_EDOC_EM_DIGITACAO(self, old_state, new_state):
        ...
        if self.state_fiscal in SITUACAO_FISCAL_SPED_CONSIDERA_CANCELADO:
            # the legacy code raised the translated string itself (hence a
            # TypeError instead of a proper error message).
            raise UserError(
                _(
    ...
    def _after_change_state(self, old_state, new_state):
        ...
        Também dispara os callbacks `after` declarados pela máquina de estados
        para a transição em questão.
        ...
        self._run_fsm_callbacks(old_state, new_state, "after")
    ...
    def _change_state(self, new_state, force_change=False):
        ...
        Este é o ponto único de escrita de state_edoc: tanto os botões
        legados quanto os gatilhos da máquina de estados (:meth:`_trigger_fsm`)
        passam por aqui, de modo que o estado só é gravado quando a transição
        realmente ocorre.
    ...
    def _document_confirm_to_send(self):
        # only draft documents are confirmed: confirming a document that is
        # already confirmed (or authorized) is a no-op instead of an invalid
        # transition error, since account.move._post() may confirm the same
        # document again (see OCA/l10n-brazil#4364).
