...
class DocumentLine(models.Model):
    ...
    def prepare_line_servico(self):
        return {
            'valor_servicos': round(float(self.amount_fiscal_operation), 2),
            ...
            'valor_ir_retido': round(float(self.irpj_wh_value), 2),
            'valor_csll': round(float(self.csll_value), 2),
            'valor_csll_retido': round(float(self.csll_wh_value), 2),
            ...
            'valor_liquido_nfse': round(float(self.amount_financial), 2),
            'item_lista_servico':
