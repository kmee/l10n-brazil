...
from ..constants.mdfe import (BODY_TYPE, EMISSION_TYPE_MDFE_CONTINGENCIA,
                              EMISSION_TYPE_MDFE_NORMAL,
                              EMISSION_TYPE_MDFE_PROPRIA, EMITTER_TYPE,
                              FISCAL_MODEL_MDFE, MDFE_ENVIRONMENT_PRODUCAO,
                              SITUATION_MDFE_ENCERRADA, SITUATION_NFE,
                              SITUATION_NFE_A_ENVIAR, SITUATION_NFE_AUTORIZADA,
                              SITUATION_NFE_CANCELADA,
                              SITUATION_NFE_EM_DIGITACAO,
                              SITUATION_NFE_ENVIADA, SITUATION_NFE_REJEITADA,
                              STATUS_MDFE, TRANSPORT_MODALITY,
                              TRANSPORT_MODALITY_RODOVIARIO, TRANSPORTER_TYPE,
                              WHEELED_TYPE)
...
class SpedDocumento(models.Model):
    ...
    def _onchange_fiscal_operation_id(self):
        ...
        if self.fiscal_operation_id.document_type == FISCAL_MODEL_MDFE:
            ...
            result['value']['transporter_type'] = \
                self.fiscal_operation_id.transporter_type
            result['value']['transport_modality'] = \
                self.fiscal_operation_id.transport_modality
            self.situation_mdfe = SITUATION_NFE_EM_DIGITACAO

        return result

    @api.onchange('company_id', 'document_type', 'edoc_purpose', 'serie', 'environment_mdfe')
    def _onchange_serie(self):
        ...
        ultimo_numero = self.search([
            ...
            ('environment_mdfe', '=', self.environment_mdfe),
    ...
    def _confirma_documento(self):
        ...
        for record in self:
            route_estate_ids = \
                record.item_mdfe_ids.mapped('receiver_city_id').mapped('state_id')
    ...
    def _envia_documento(self):
        ...
        if resposta.cStat != '103':
            self.xml_error_message = 'Código de retorno: ' + resposta.cStat + \
        ...
        if recibo.cStat == '105':
            ...
            #
            self.xml_error_message = 'Código de retorno: ' + \
    ...
    def _grava_anexo(self, nome_arquivo='', conteudo='',
                     tipo='application/xml', model='l10n_br_fiscal.document'):
        self.ensure_one()
