...
# CST 000 (fully taxed) and its matching cClassTrib, used when the line has no
# IBS/CBS CST or tax classification of its own.
...
> <det>
    ...
    def _build_nfe40_ibscbs(self):
        """Build the IBSCBS group of the item.

        Lines with no IBS/CBS amount (exempt, immune, not taxed) still carry
        the group with CST and cClassTrib: the gIBSCBS choice is optional in
        the schema, so only lines with amounts get it.
        """
        self.ensure_one()
        ...
        return TtribNfe(**_binding_kwargs(TtribNfe, values))
