...
class TestCnabFile(TransactionCase):
    ...
    def test_cnab_file_import(self):
        cnab_file_path = get_module_resource(
            ...
            "tests/test_cnab_file/",
            "cnab240segmentoE.txt",
        )
        cnab_file = base64.b64encode(open(cnab_file_path, "rb").read())
        bank_statement = self.absi_model.create(dict(data_file=cnab_file))
        bank_statement.import_file()
        bank_st_record = self.abs_model.search([("name", "like", "000001405431")])[0]
        ...
        line = self.absl_model.search(
            [("name", "ilike", "RESGATE"), ("statement_id", "=", bank_st_record.id)]
        )[0]
