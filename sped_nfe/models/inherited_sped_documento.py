        ...
        if self.modelo == MODELO_FISCAL_NFE:
            ...
            if self.empresa_id.modelo_danfe:
                ...
                modelo_danfe.write(base64.b64decode(
                    self.empresa_id.modelo_danfe))
                modelo_danfe.seek(0)
                processador.danfe.template = modelo_danfe

            processador.danfe.nome_sistema = 'Taŭga Haveno 11.0'
            ...
            if self.empresa_id.modelo_danfce:
                modelo_danfe = self.empresa_id.modelo_danfce
                processador.danfce.template = BytesIO().write(modelo_danfe)

            processador.danfce.nome_sistema = 'Taŭga Haveno 11.0'
            processador.caminho = os.path.join(
                self.empresa_id.caminho_sped,
                'nfce',
            )
