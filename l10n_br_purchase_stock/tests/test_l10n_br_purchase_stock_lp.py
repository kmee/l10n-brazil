...
class L10nBrPurchaseStockBase(L10nBrPurchaseStockBase):
    def setUp(self):
        ...
        # Set ICMS, IPI, PIS, COFINS as non-creditable for Compra para industrialização.
        #   Note that any purchase using this operation must have it's tax values included
        #   in the stock cost.
        self.fol_ind = self.env.ref("l10n_br_fiscal.fo_compras_compras")
        self.fol_ind.non_creditable_tax_definition_ids += self.env.ref(
    ...
    def test_purchase_order_lp_stock_price(self):
        ...
        computed_stock_price = line1.stock_price
        self.assertEqual(
            expected_stock_price, computed_stock_price, "Stock value mispriced"
        )

        # line 1 -> 120 (price with taxes)
        expected_stock_price = line2.amount_total / line2.product_qty
        computed_stock_price = line2.stock_price
        self.assertEqual(
            expected_stock_price, computed_stock_price, "Stock value mispriced"
        )
        ...
        cost1_diff = float_is_zero(
            product_id1.standard_price - 90.35, precision_rounding=0.001
        )
        cost2_diff = float_is_zero(
            product_id2.standard_price - 120, precision_rounding=0.001
        )
        self.assertEqual(
            cost1_diff,
            True,
