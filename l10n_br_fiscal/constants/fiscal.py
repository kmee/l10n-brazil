...
DOCUMENT_STATE_DRAFT = SITUACAO_EDOC_EM_DIGITACAO
DOCUMENT_STATE_OPEN = SITUACAO_EDOC_A_ENVIAR
...
# DEPRECATED: the allowed state_edoc transitions are now declared by the state
# machine of l10n_br_fiscal_edi (see get_state_machine_config()). These tuples
# are kept exported for backward compatibility with third party modules and
# they are still the reference the state machine is built upon.
