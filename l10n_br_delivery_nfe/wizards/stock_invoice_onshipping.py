...
class StockInvoiceOnshipping(models.TransientModel):
    ...
    def action_generate(self):
        ...
        pick_ids = self.vol_ids.filtered(lambda v: not v.picking_id.has_vol_ids).mapped(
            "picking_id"
        )
        for pick_id in pick_ids:
            volumes = self.vol_ids.filtered(lambda v: v.picking_id == pick_id).mapped(
                "nfe40_qVol"
            )
            pick_id.number_of_volumes = sum([float(v) for v in volumes])
        return res
