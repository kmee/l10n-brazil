...
- `tribISSQN` mapped from `issqn_eligibility` by the national `TSTribISSQN`
  enum, distinct from the ABRASF table. Export of service, ISSQN exemption
  and suspended liability carry their own group (`comExt`, `BM`, `exigSusp`);
  the send is blocked with a readable message when the group is incomplete.
